
import './App.css'

import Calendar from "./components/Calendar/Calendar.tsx";
import {useEffect, useState} from "react";
import styled from 'styled-components'
import { createPortal } from 'react-dom';
import {formatDate} from "./utils/helpers/date";

interface IColor {
    color: 'green' | 'orange' | 'grey'
}

const ColorContent = styled.button<IColor>`color: ${({color = 'green'})=> color}`

function App() {
    const [selectedDate, selectDate] = useState(new Date());
    const [color, selectColor] = useState('');
    const [colorDays, setColorDays ] = useState([{key: '', value: ''}]);

   useEffect(() => {
        const key = formatDate(selectedDate, 'DD MMM')
        setColorDays([{key: key, value: color}])
       
    }, [setColorDays]);

    console.log(colorDays, 'colorDays')
    const [showModal, setShowModal] = useState(false);

    return (
    <>
        <div className="app__container">
            <div className="app__container-header">
                <button className="button-arrow" >
                    <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6 12H18M6 12L11 7M6 12L11 17" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                </button>
                <div className='date__container'>
                    <button onClick={() => setShowModal(true)}>
                        <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M3 9H21M7 3V5M17 3V5M6 12H8M11 12H13M16 12H18M6 15H8M11 15H13M16 15H18M6 18H8M11 18H13M16 18H18M6.2 21H17.8C18.9201 21 19.4802 21 19.908 20.782C20.2843 20.5903 20.5903 20.2843 20.782 19.908C21 19.4802 21 18.9201 21 17.8V8.2C21 7.07989 21 6.51984 20.782 6.09202C20.5903 5.71569 20.2843 5.40973 19.908 5.21799C19.4802 5 18.9201 5 17.8 5H6.2C5.0799 5 4.51984 5 4.09202 5.21799C3.71569 5.40973 3.40973 5.71569 3.21799 6.09202C3 6.51984 3 7.07989 3 8.2V17.8C3 18.9201 3 19.4802 3.21799 19.908C3.40973 20.2843 3.71569 20.5903 4.09202 20.782C4.51984 21 5.07989 21 6.2 21Z" stroke="#000000" strokeWidth="2" strokeLinecap="round"/>
                        </svg>
                    </button>
                    {formatDate(selectedDate, 'DD MMM')}
                </div>
                <button className="button-arrow" >
                    <svg width="24px" height="24px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M6 12H18M18 12L13 7M18 12L13 17" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
                </button>
            </div>


            {showModal && createPortal(
                <Calendar color={color} selectedDate={selectedDate} selectDate={selectDate} onClose={() => setShowModal(false)} />,
                document.body
            )}

        </div>
        <div className='todo__container'>

            <ColorContent color={'green'} onClick={() => selectColor('green')}>Hello World</ColorContent>
            <ColorContent color={'orange'} onClick={() => selectColor('orange')}>Hello World</ColorContent>
            <ColorContent color={'grey'} onClick={() => selectColor('grey')}>Hello World</ColorContent>

        </div>

    </>
  )
}

export default App
